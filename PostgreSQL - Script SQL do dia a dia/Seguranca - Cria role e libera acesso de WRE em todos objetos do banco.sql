/************************************************************************************************************
										ACESSO LEITURA E ESCRITA MAIS EXECUÇÃO.
************************************************************************************************************/
DO 
$$
-- Paramentros: 
DECLARE 
	nmRole varchar := 'rl_dev_rwe_stg1g01'; 
...
/* Se a regra existi iniciar a adinção dos acessos. */	
	...
	-- Base de dados. 
		query = 'GRANT TEMPORARY, CONNECT ON DATABASE '|| vrBase ||' TO '|| nmRole ||';';
			EXECUTE query; 

		query = 'ALTER DEFAULT PRIVILEGES FOR ROLE postgres
		GRANT INSERT, UPDATE, DELETE, SELECT ON TABLES TO '|| nmRole ||'; ';
			EXECUTE query; 
	...
	ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA '|| B.schema_name ||'
	GRANT ALL ON SEQUENCES TO '|| nmRole ||'; 
	...
	-- Tabelas
		...
		LOOP
			query = 'GRANT INSERT, UPDATE, DELETE, SELECT ON TABLE ' || B.table_schema ||'.'||B.table_name ||' TO '|| nmRole ||';';
			--RAISE NOTICE '%', query; 
			EXECUTE query;
		...
		FOR B IN SELECT n.nspname,p.proname, p.oid FROM pg_catalog.pg_proc p
			LEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
				   WHERE n.nspname not like 'information_schema'
					 ...
					 AND P.Prokind = 'f'
		LOOP
			query = 'GRANT EXECUTE ON FUNCTION ' || B.nspname ||'.'||B.proname ||'('||pg_get_function_arguments(B.oid) ||') TO '|| nmRole ||';';
			--RAISE NOTICE '%', query; 
			EXECUTE query;
		...
		FOR B IN SELECT n.nspname,p.proname, p.oid FROM pg_catalog.pg_proc p
			LEFT JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
				   WHERE n.nspname not like 'information_schema'
					 ...
					 AND P.Prokind = 'p'
		LOOP
			query = 'GRANT EXECUTE ON PROCEDURE ' || B.nspname ||'.'||B.proname ||'('||pg_get_function_arguments(B.oid) ||') TO '|| nmRole ||';';
			--RAISE NOTICE '%', query; 
			EXECUTE query;
	...
	-- materialized 
		...
		LOOP
			query = 'GRANT INSERT, UPDATE, DELETE, SELECT ON TABLE ' || B.schemaname ||'.'||B.matviewname ||' TO '|| nmRole ||';';
			--RAISE NOTICE '%', query; 
			EXECUTE query;
